...
export const handleAddTodo = async (req: RequestWithBody<TaskRequestBody>, res: Response, next: NextFunction) => {
  const { title, note, isDone, id } = req.body;
  const userId = req.user.id;

  let sql: string;

  if (!id) {
    sql = `INSERT INTO todos (title, note, dateCreated, isDone, userId) VALUES ($1, $2, $3, $4, $5)`;
    try {
    ...
    sql = `UPDATE todos SET title = ($1), note = ($2), dateCreated = ($3), isDone = ($4) WHERE userid = ($5) AND id = ($6)`;
