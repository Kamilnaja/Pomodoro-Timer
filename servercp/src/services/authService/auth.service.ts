...
const checkPassword = async (
  ...
  try {
    ...
    if (isPasswordCorrect) {
      ...
      res.json({
        token: jwt.sign({ login, email, id }, accessTokenSecret),
      });
    } else {
      res.status(401).send({
...
export const authenticateJWT = (req: Request<Registration>, res: Response<LoginResponse>, next: () => void) => {
  ...
  if (authHeader) {
    ...
    jwt.verify(
      token,
      accessTokenSecret,
      (err: JsonWebTokenError | NotBeforeError | TokenExpiredError | null, user: object | undefined) => {
        if (err) {
          return res.sendStatus(403);
        }
        req.user = user as Registration;
        next();
      },
    );
  } else {
